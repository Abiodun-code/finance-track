...
const Main = () => {
  ...
  const loginToken = useSelector<RootState, string | null>((state) => state.loginAccount.accessToken);
  ...
  return (
    <NavigationContainer>
      {isAuthenticated === null ? null : isAuthenticated ? (<Authenticated />) : (<NotAuthenticated />)}
    </NavigationContainer>
  );
};
